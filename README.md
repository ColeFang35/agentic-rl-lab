...
## 六、实测结果（RTX 4090D · Qwen2.5-7B-Instruct · 160 题 × 4 条 = 640 条轨迹）

**采样分布**：分数均值 0.357 / 中位数 0.400；满分 0 条、≥0.7 有 263 条
→ 用 **≥0.7 作为 RFT 种子阈值**（拒绝采样的标准做法，不苛求满分）

**训练**
```
RFT : trainable% 0.2643   loss 0.0287 → 0.0082   382s（263 条样本）
DPO : trainable% 0.2643   rewards/accuracies 0.775 → 1.0   rewards/margins 0.317 → 1.519（47 对）
```

**测试集（40 题）对比 —— 同一套任务、贪心解码，只换模型**

| 模型 | 达标率(≥0.7) | 平均奖励 | 工具名正确率 |
|---|---|---|---|
| 基座 | 42.5% | 0.341 | 52.5% |
| **RFT（拒绝采样 + SFT）** | **62.5%**（+20.0pt） | **0.457** | **67.5%**（+15.0pt） |
| **DPO** | **52.5%**（+10.0pt） | **0.452** | **70.0%**（+17.5pt） |

**样例（基座 → DPO）**
```
问：帮我退掉 SO20260903011
基座：请告诉我您退货的原因，以便我们为您处理退款申请。      ← 根本没调工具
DPO ：<tool_call>{"name": "check_refund_policy", ...}      ← 正确调用工具
```

> 结论：**拒绝采样**最直接地提升"达标率"，**DPO** 最明显地提升"工具名正确率"。
> 完整数据见 `reports/rl_compare.json`。
